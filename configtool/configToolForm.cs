    ...
    public partial class FormConfig : Form
    {
            ...
            for (i = 0; i < cfg.getNumItems(); i++)
            {
                ...
                if(cfg.getItem(i).ble != null)
                {
                    row = new String[] { cfg.getItem(i).tag,
                                              ...
                                              cfg.getItem(i).ble.r ? "R" : "-",
                                              cfg.getItem(i).ble.w ? "W" : "-",
                                              cfg.getItem(i).ble.n ? "N" : "-"
                                            };
                }
                else
                {
     ...
     //       int i = 0;
      //      String[] row;
        ...
        {
                    Debug.Print("edit "+e.RowIndex+" "+e.ColumnIndex);
            ...
            foreach (DataGridViewRow row in dataGridViewConfig.Rows)
            {
                if (row.Index < dataGridViewConfig.RowCount)
                {
                    ...
                    if (row.Cells["r"].Value != null)
                        rValue = row.Cells["r"].Value.Equals("R") ? true : false;
                    else
                        rValue = false;

                    if (row.Cells["w"].Value != null)
                        wValue = row.Cells["w"].Value.Equals("W") ? true : false;
                    else
                        wValue = false;

                    if (row.Cells["n"].Value != null)
                        nValue = row.Cells["n"].Value.Equals("N") ? true : false;
                    else
                        nValue = false;
...
void eraseConfig(void);
      ...
      //      bLEServiceExportDlg.info = cfg.serviceExportInfo;
        ...
        { 
            ...
            String srvNameTag = ("BLE_" + srvName + "_").ToUpper();
...
{
  TI_BASE_UUID_128("+srvNameTag.ToUpper()+@"UUID)
};
...
#define " + srvNameTag + "ATTR_TABLE_SIZE     " + (index-1) + @"
...
";
        ...
        void exportServiceCode(String template, String outsrc, String[] tags)
        {
            String outline;
            String[] outsrclines;
            List<String> outlines = new List<String>();
            foreach (string line in System.IO.File.ReadLines(template))
            {
                outline = exportTagReplacer(tags, line);
                System.Console.WriteLine(outline);
                outlines.Add(outline);
            }

            outsrclines = outlines.ToArray();

            System.IO.File.WriteAllLines(outsrc, outsrclines);
        }

        string exportTagReplacer(String[] tags, String line)
        {
            int i;
            string outline = line;
            ...
            {
                while (line.Contains(tags[i]))
                {
                    line = outline = line.Replace(tags[i], tags[i + 1]);
                }
        ...
        {
            Debug.Print("cell content click " + e.RowIndex + " " + e.ColumnIndex);

            if(e.ColumnIndex >= 5)
            {
                toggleRWN(e.RowIndex, e.ColumnIndex);
                refreshGridView();
            }
        ...
        {
            Debug.Print("cell click " + e.RowIndex + " " + e.ColumnIndex);
            if (e.ColumnIndex >= 5)
            {
                toggleRWN(e.RowIndex, e.ColumnIndex);
                refreshGridView();
        ...
        private void toggleRWN(int row, int col)
        {
            configItem item = cfg.getItem(row);

            switch (col)
            {
                case 5:
                    item.ble.r = !item.ble.r;
                    break;

                case 6:
                    item.ble.w = !item.ble.w;
                    break;

                case 7:
                    item.ble.n = !item.ble.n;
                    break;
            }
        }
    }
